...
//  NEW Lesson "Basic Loops in Solidity" date = 10, june 2022, time = 7:10
    ...
    // "MODIFIER" = is a keyword which we can add in function declaration, So it can modify the function with that functionality
    modifier onlyOwner{
        require (msg.sender == owner, "Withdrawal is not owner");
        _;
    }

    // So, whats happening here ?
    // We put this "onlyOwner" on withdraw function and before executing the whole withdraw function it will check the modifer
    // we put on function and do what the modifer tells him then " _; " this underscore represents do everything next
    // meanns = it will check modifer first then execute whole function
    // if that underscore is above, require function then whole withdraw function runs then it will check the MOdifier!
}
